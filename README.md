...
- **Cancelling and carrying on** – A running review can be stopped. A cancelled or failed review can be **carried on from where it stopped, as the same job**: what already finished keeps its verdict and only the items still unjudged are reviewed, so neither the money nor the time is thrown away. No second job appears in the history.
...
- **History by department** – Reviews are tied to a department. **A regular user sees the reviews they started plus the ones from their own department; an administrator sees everything.** A review run by someone who belongs to no department carries no department, so only the person who started it can see it. Reviews from your department can be read, but only the person who ran one can change its verdicts, re-review, cancel or delete it. Checklists are shared tools, so anyone in the same department can also edit them; only the creator or an administrator can delete one, and the last editor is recorded. Someone in more than one department chooses which one a review is for. Membership comes from the identity provider (Cognito groups, or attributes when federated over SAML).

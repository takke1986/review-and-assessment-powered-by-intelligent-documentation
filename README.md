...
- **History by department** – Reviews are tied to a department. **A regular user sees the reviews they started plus the ones from their own department; an administrator sees everything.** A review run by someone who belongs to no department carries no department, so only the person who started it can see it. Reviews from your department can be read, but only the person who ran one can change its verdicts, re-review, cancel or delete it. Someone in more than one department chooses which one a review is for. Membership comes from the identity provider (Cognito groups, or attributes when federated over SAML).

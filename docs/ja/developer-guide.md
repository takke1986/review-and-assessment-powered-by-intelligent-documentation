...
続きから流すときは、同じジョブの状態を `pending` に戻して待ち行列へ入れ直します（`reopenJob`）。中止を守る条件付き書き換えをあえて越える唯一の経路で、越えられるのは `failed` と `cancelled` からだけです。キューは本文が同じメッセージを重複と見なして捨てるため、投入時に毎回異なる `MessageDeduplicationId` を渡します。これを忘れると、続きから流したつもりで何も起きません。

集約の直前に、判定の付いていない項目が残っていないかを確かめます（`review-completeness.ts`）。残っていれば完了にせず失敗にします。

審査の前に、書類を読み取る段が入ります（`PlanReading` → `ReadDocuments`(Map) → `StoreReading`）。1回の呼び出しに渡せるページ数（100）と画像の枚数（20）は引き上げられないので、区切って読み、結果を S3 に置きます。1回で渡せて文字も取れる書類には回らないので、普段の審査に費用も時間も足しません。読み取りは審査の補助なので、失敗しても審査は続きます（3つの状態から `ProcessAllItems` へ catch）。

読み取り結果は**審査ジョブごと**に置き、ジョブをまたいで使い回しません。読み取りはそのジョブのチェックリストの観点を踏まえて書くため、別の審査に持ち越すと観点がずれたものを根拠にすることになります。

1つのチェック項目で実際に目で見られる画像は `MAX_IMAGES_PER_REVIEW`（既定 5）枚までです。超えると道具が断り、審査は「見た範囲で」続きます。何枚見たか・断ったかは `reviewMeta` に残り、画面に「一部だけを見た判定」と出ます。
...
| そのジョブのまま続きから流せるか | 同 `review-resume.ts` |
| 読み取りに渡す「見るべき観点」 | `backend/src/review-workflow/review-processing/focus.ts` |

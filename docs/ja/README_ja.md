...
- **AI によるチェックリスト抽出** — 規程・ガイドライン・仕様書などをチェックリストへ自動変換します。PDF に加えて **Excel・Word・PowerPoint・テキスト（.txt / .md / .csv）** も取り込めます。Office ファイルは中身が XML なので画像化を経ずにそのまま読み、表の値も数式も欠けません。
...
- **判定の手直しと、その記録** — AI の判定は人が覆せます。覆すと **AI が何と判定したか・誰がいつ覆したか・なぜ覆したか**（基準の読み違い／文書にあるのに見つけられなかった／項目が曖昧、など）が残ります。誰が決めたのかが後から辿れます。
- **傾向と着眼点** — チェック項目ごとに、AI の判定が人に覆された記録を集計し、**「AI が厳しすぎる」「AI が見落としている」**を見分けます。項目に「着眼点」を書き添えると次の審査から効き、**書く前と後で覆された率がどう変わったか**まで表示します。
- **審査の中止と、続きからの再開** — 走っている審査を途中で止められます。止めた審査・失敗した審査は、**そのジョブのまま続きから流せます**。済んだ項目は判定を残したまま、まだ見ていない項目だけを審査するので、費用と時間を捨てずに済みます。**別のジョブは作りません**（同じ書類の審査が履歴に2行並ばないため）。
- **再審査** — 不合格だった項目について、書類を差し替えて審査し直せます。差し替えなかった項目の結果は引き継がれます。
- **結果の持ち出し** — 審査結果は3つの形で外に出せます。**印刷用レポート**（ブラウザの印刷から PDF 保存。顧客に渡す想定で、社内の URL は載せません）、**文章のコピー**（メール文の下書きを生成 AI に書かせる用）、**CSV**（表計算ソフトでの並べ替え・集計用）。並び順と通し番号は3つで揃えてあり、紙とファイルを突き合わせても話が合います。
- **完了の通知** — 審査が終わるとトーストで知らせます。別のタブや別のアプリを見ているときは OS の通知も出ます。
- **費用の見える化** — 審査1件あたりの費用に加えて、**期間ごとの合計・月ごとの推移・チェックリスト別の内訳・費用の高い審査**を表示します。どの審査に費用が掛かっているかが分かると、項目数やモデルの見直しにつながります。
- **部署ごとの履歴** — 審査を部署に紐づけます。**一般の利用者は自分が開始した審査と自分の部署の審査だけ、管理者は全部**が見えます。どの部署にも属していない人が回した審査には部署が付かないので、その場合は開始した本人だけが見られます。同じ部署の審査は内容を読めますが、判定の変更・再審査・中止・削除は作成者だけです。チェックリストは部署で使い回す道具なので、同じ部署の人も直せます（削除は作成者と管理者だけ。最後に直した人を記録します）。兼務の人は、審査を作るときにどの部署の仕事かを選びます。所属は認証基盤（Cognito のグループ、または SAML 連携時の属性）から取ります。

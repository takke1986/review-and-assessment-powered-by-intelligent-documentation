...
- **部署ごとの履歴** — 審査を部署に紐づけます。**一般の利用者は自分が開始した審査と自分の部署の審査だけ、管理者は全部**が見えます。どの部署にも属していない人が回した審査には部署が付かないので、その場合は開始した本人だけが見られます。同じ部署の審査は内容を読めますが、判定の変更・再審査・中止・削除は作成者だけです。兼務の人は、審査を作るときにどの部署の仕事かを選びます。所属は認証基盤（Cognito のグループ、または SAML 連携時の属性）から取ります。
